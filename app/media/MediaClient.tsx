...
function formatUTC(dt: string) {
    if (!dt) return "";
    return moment(dt).fromNow();
}

const tags = ["All", "Analysis", "Webinars", "Tutorials", "Live Trading", "Interviews"];
...
function MediaContent() {
    ...
    const playingVideoId = searchParams.get("v");

    const currentVideo = useMemo(() =>
        videos.find(v => v.uid === playingVideoId) || null
        , [playingVideoId]);
    ...
    const filteredVideos = useMemo(() => {
        ...
        if (selectedTag === "All" && !searchQuery && featuredVideo) {
            result = result.filter(v => v.uid !== featuredVideo.uid);
        }

        return result;
    }, [selectedTag, searchQuery, featuredVideo]);

    const recommendations = useMemo(() =>
        videos.filter(v => v.uid !== playingVideoId)
        , [playingVideoId]);

    const handlePlayVideo = (id: string) => {
        const newUrl = `${pathname}?v=${id}`;
    ...
    const handleShare = async () => {
        ...
        const shareUrl = `${window.location.origin}${pathname}?v=${currentVideo.uid}`;
        if (navigator.share) {
            try {
            ...
            <div className="pt-28 container mx-auto px-6 flex-1 flex flex-col">
                {!playingVideoId ? (
                    <>
                            ...
                            <div className="flex items-center gap-3 overflow-x-auto w-full md:w-auto pb-4 md:pb-0 scrollbar-hide">
                                {tags.map((tag) => (
                                    <button
                                        ...
                                        className={`whitespace-nowrap px-5 py-2 rounded-full text-sm font-medium transition-all cursor-pointer ${selectedTag === tag
                                            ? "bg-white text-black"
                        ...
                        {selectedTag === "All" && !searchQuery && featuredVideo && (
                            <ScrollReveal variant="fade-up">
                                <div className="mb-12 group cursor-pointer" onClick={() => handlePlayVideo(featuredVideo.uid)}>
                                    <div className="relative w-full aspect-video md:aspect-[21/9] rounded-[2rem] overflow-hidden mb-6 border border-white/10 group-hover:border-primary/50 transition-all duration-500 shadow-2xl">
                                        ...
                                        <div className="absolute bottom-6 right-6 bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-lg text-sm font-bold tracking-wider">
                                            <VideoDuration src={featuredVideo.videoUrl} />
                                        </div>
                                    </div>

                                    <div className="flex gap-6 items-start">
                                        <div className="relative h-14 w-14 rounded-full overflow-hidden flex-shrink-0 border border-white/20">
                                            <Image src={featuredVideo.author.avatar} alt={featuredVideo.author.name} fill unoptimized className="object-cover" />
                                        </div>
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-2 text-muted-foreground mb-3 text-sm md:text-base">
                                                ...
                                                <span className="w-1 h-1 rounded-full bg-white/20 mx-1" />
                                                <span>{formatUTC(featuredVideo.datetime)}</span>
                                            </div>
                        ...
                        <div className="pb-24">
                            <h3 className={`${spaceGrotesk.className} text-2xl font-bold mb-8`}>
                                {selectedTag === "All" ? "Recommended For You" : `${selectedTag} Videos`}
                            </h3>
                            ...
                            {filteredVideos.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12 transition-all duration-500">
                                    {filteredVideos.map((video) => (
                                        <div key={video.uid} className="group cursor-pointer active:scale-95 transition-transform duration-200" onClick={() => handlePlayVideo(video.uid)}>
                                            <div className="relative w-full aspect-video rounded-2xl overflow-hidden mb-4 border border-white/5 group-hover:border-primary/30 transition-all duration-300 shadow-lg">
                                                <Image src={video.thumbnail} alt={video.title} fill unoptimized className="object-cover group-hover:scale-105 transition-transform duration-500" />
                                            ...
                                            <div className="flex gap-4 items-start">
                                                <div className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0 border border-white/10 mt-1">
                                                    <Image src={video.author.avatar} alt={video.author.name} fill unoptimized className="object-cover" />
                                                </div>
                                                <div>
                                                    ...
                                                    <div className="text-sm text-muted-foreground flex flex-col gap-0.5">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span>{formatUTC(video.datetime)}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="px-2">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 py-4 border-b border-white/5">
                                    <div className="flex items-center gap-4">
                                        <div className="relative h-12 w-12 rounded-full overflow-hidden border border-white/10">
                                            <Image src={currentVideo?.author.avatar || ""} alt={currentVideo?.author.name || ""} fill unoptimized className="object-cover" />
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button onClick={handleShare} className="flex items-center gap-2 bg-white/5 px-5 py-2.5 rounded-full border border-white/5 hover:bg-white/10 transition-all active:scale-95 cursor-pointer">
                                ...
                                <div className="bg-white/5 p-6 rounded-3xl border border-white/5 mb-8">
                                    <div className="flex gap-4 text-sm font-bold mb-2">
                                        ...
                                        <span>{currentVideo ? formatUTC(currentVideo.datetime) : ""}</span>
                            ...
                            <div className="flex flex-col gap-4">
                                {recommendations.map((video) => (
                                    <div
                                        ...
                                        onClick={() => handlePlayVideo(video.uid)}
                                    >
                                        <div className="relative w-44 aspect-video rounded-xl overflow-hidden flex-shrink-0 border border-white/5 group-hover:border-primary/30 transition-all">
                                            <Image src={video.thumbnail} alt={video.title} fill unoptimized className="object-cover group-hover:scale-110 transition-transform duration-500" />
